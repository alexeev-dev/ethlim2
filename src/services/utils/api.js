...
export function withRouteParams(path, params) {
  ...
  const slashEnd = pathChunks[pathChunks.length - 1].length === 0
  return '/' + path.split('/')
    .map(trim)
    .filter(notEmpty)
...
export function makeQueryUrl(route, routeParams = {}, queryParams) {
  ...
  const endpoint = chunks.length > 1
    ...
    : chunks.length > 0
      ...
      : null
  if (!endpoint) {
    throw new Error(`makeQueryUrl(): wrong route "${route}"!`)
  }
...
function encodeParam(name, value, isArray = false) {
  ...
  const encodedValue = encodeURIComponent(value).replace(/%3B/, ',')
  return `${encodedName}=${encodedValue}`
}

function encodeParamMap(name) {
  return (value) => encodeParam(name, value, false)
}
...
function replaceParams(params, path) {
  return (chunk) => {
    if (chunk.startsWith(':')) {
      ...
      throw new Error(
        `withRouteParams(): parameter not set "${chunkName}"` +
        `for ${path}.\n Check that you have correct parameters`
      )
    }
    return chunk
  }
...
function makeHeaders(method, token, bodyParams) {
  const authHeader = token ? {'Authorization': `Token ${token}`} : null
...
function getHost() {
  ...
  if (window.location.host === 'dev.cloudmind.ru' || window.location.host === 'localhost:3000') {
    return host.dev
  }
  return host.prod
}
